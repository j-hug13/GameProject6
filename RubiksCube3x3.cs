    ...
    public class RubiksCube3x3
    {
        ...
        public SaveCubeState GetState(double solvingTime)
        {
            SaveCubeState state = new SaveCubeState()
            {
                ...
                CubeletPositions = new Point3[3][][],
                CubeletColors = new ColorSaveFormat[3][][][]
            };

            for (int x = 0; x < 3; x++)
            {
                state.CubeletPositions[x] = new Point3[3][];
                state.CubeletColors[x] = new ColorSaveFormat[3][][];

                for (int y = 0; y < 3; y++)
                {
                    state.CubeletPositions[x][y] = new Point3[3];
                    state.CubeletColors[x][y] = new ColorSaveFormat[3][];
